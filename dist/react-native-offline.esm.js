...
function handleOfflineAction(state, _a, comparisonFn) {
    ...
    if (isActionToRetry || isThunkToRetry) {
        ...
        if (Array.isArray(similarActionQueued)) {
            return __assign(__assign({}, state), { actionQueue: similarActionQueued });
        }
...
var isReleaseQueueRunning = false;
var createReleaseQueue = function (getState, next, delay, shouldDequeueSelector) { return function () { return __awaiter(void 0, void 0, void 0, function () {
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                if (!!isReleaseQueueRunning) return [3 /*break*/, 2];
                isReleaseQueueRunning = true;
                return [4 /*yield*/, releaseActions(getState, next, delay, shouldDequeueSelector)];
            case 1:
                _a.sent();
...
function releaseActions(getState, next, delay, shouldDequeueSelector) {
    return __awaiter(this, void 0, void 0, function () {
        var state, queue, action, _a, isConnected, isQueuePaused;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    state = getState();
                    queue = state.network.actionQueue;
                    if (!(queue && queue.length)) return [3 /*break*/, 2];
                    action = queue[0];
                    _a = state.network, isConnected = _a.isConnected, isQueuePaused = _a.isQueuePaused;
                    if (!(isConnected && !isQueuePaused && shouldDequeueSelector(state))) return [3 /*break*/, 2];
                    next(removeActionFromQueue(action));
                    next(action);
                    return [4 /*yield*/, wait(delay)];
                case 1:
                    _b.sent();
                    return [2 /*return*/, releaseActions(getState, next, delay, shouldDequeueSelector)];
                case 2: return [2 /*return*/, new Promise(function (resolve) { return resolve(true); })];
            }
        });
    });
}
    ...
    return function (_a) {
        ...
        return function (next) { return function (action) {
            ...
            if (shouldInterceptAction &&
                actionQueue.length &&
                get_1(action, 'meta.queuedEvenIfOnline', false)) {
